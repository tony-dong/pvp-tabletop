...
export function toStadium() {
   if (
      cardSelection.get().length !== 1 ||
      selectionPile === 'stadium' ||
      !selectionPile.get().length
   )
      return;
...
export function attachSelection(slot) {
   ...
   share(evolving.get() ? 'cardsEvolved' : 'cardsAttached', {
      slotId: slot.id,
      cards: ids,
      from,
   });
...
export function shareBoardstate() {
   ...
   if (deck) {
      share('boardState', { cards: deck, board: exportBoard() });
   }
}
